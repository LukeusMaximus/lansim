...
public class RouterMod extends module.ModuleAbstractClass {
    ...
    public RouterMod() {
        buffer.EditMode.module = this;// TODO: Remove this line from
                                      // here..........................IMPORTANT!
    ...
    private void servicePort(int portNo) {
        ...
        modUI.getModWin().msg.append("\nFrom:" + packet.getFromId() + "  To:"
                + packet.getToId());

        if (serialNo == packet.getToId()
                && (packet.getData().startsWith("ping"))) {
            modUI.getModWin().amsg.append("\n Ping from " + packet.getFromId());
            Packet tempPacket = new Packet(packet.getFromId(), serialNo, 10,
                    packet.getData() + packet.getData(), 0, true);
    ...
    public boolean step(double time) {
        ...
        if (noOfPorts != 0) {
            ...
            if (ports[lastServiced].hasData()
                    && !ports[lastServiced].isTransmiting())
