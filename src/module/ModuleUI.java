...
public class ModuleUI extends ComponentUI implements ModuleWinUI {
    ...
    private Point coord = null; // The coordinate of the module's upper-left
                                // corner.
    ...
    /**
     * Must be overridden by deriving class so as to display a custom property
     * tab in the properties dialog of the module. If this is not overriden
     * (i.e. null is returned) then no custom property tab is shown.
     * 
     * @return The JPanel object with all the custom buttons and fields in it.
     *         The events generated on this must be handeled by the module
     *         itself.
     */
    ...
    /**
     * Returns true if PropertyPage available for the give mode
     * (SIMULATION_MODE, EDIT_MODE, etc.) else false.
     * 
     * @param mode
     * @return
     */
    ...
    /**
     * Override this if your module needs to update its state while simulating
     * in SIMULATION_MODE or PAUSED_MODE or EDIT_MODE.
     * 
     * @param mode
    ...
    /**
     * Used by setpSimulation to get a new instance of DataUI. If the module
     * writer just creates a new subclass of DataUI then he must override this
     * method to return object its custom DataUI (sub) class. If the module
     * writer overrides also setpSimulation then instead of re-writing the codes
     * in stepSimulation he can just override this method too (as described
     * above) and make a call to super.setpSimulation().
     * 
     * @return
     */
    ...
    /**
     * All rendering of visuals go here. Override this to create your own module
     * with custom looks.
     * 
     * @param g
     * @param CallingObj
    ...
    public void render(Graphics g, ManagerUI CallingObj) {
        ...
        if (image == null) {// Syncronisingly loading the image.
            ...
            // image =
            // Toolkit.getDefaultToolkit().getImage("module/"+module.getName()+"/icon.gif");
        ...
        g.drawString(module.getName() + module.getSno(),
                ((int) (coord.x + width / 2)) + 15, (int) coord.y + height / 2);
        if (status == SELECTED) {
            g.setColor(Color.blue);
            g.drawRect((int) coord.x - 2, (int) coord.y - 2, width + 4,
                    height + 4);

        }
        if (status == MOVING) {
            g.setColor(Color.black);
            g.drawRect((int) coord.x + width + 5, (int) coord.y + height + 5,
                    5, 5);
    ...
    public Point2D.Double getDoubleCoord() {
        // return new
        // Point2D.Double(coord.x+((double)height)/2,coord.y+((double)width)/2);
        return new Point2D.Double(coord.x + ((double) width) / 2, coord.y
                + ((double) height) / 2);
    }

    public boolean isClicked(Point pt) {
        return (new Rectangle((int) coord.x, (int) coord.y, width, height))
                .contains(pt);
