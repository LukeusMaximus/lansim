...
/**
 * This module is to be used for testing and demonstrating Stop-and-Wait ARQ
 * protocol.<br>
 * <i>(Ref: Data and Computer Communication; William Stallings; 7th Ed.)</i><br>
 * The weird <b>SnWwA</b> stands for <b>S</b>top a<b>n</b>d <b>W</b>ait
 * <b>w</b>ith <b>A</b>cknowledgement.
 * 
 * @author Nirupam
 * 
 */
public class SnWwAMod implements module.Module {
    ...
    int sendTimer = 90;
    int noOfPorts = 0;
    private int timeoutTimer; // timeoutTimer counts down. It then resets to
                              // sendTimer value.
    ...
    private boolean stepPhysical(double currtime) {
        ...
        for (int i = 0, portNo; i < oPhysicalBuffer.size(); i++) {
            ...
            if (ports[portNo] == null)
                ...
                newPacket = true;
                modUI.getModWin().msg.append("\nPhysical-Put packet on port "
                        + portNo + "  to " + oPhysicalBuffer.get(i).getToId());
                oPhysicalBuffer.remove(i);
            }
        }
        return true;
        ...
        if (canEmitData) { // TRANSMITTER MODE
            ...
            if (packet != null && packet.getToId() == serialNo) {
                if (!packet.isCorrupt() && packet.getData() != null
                    ...
                    modUI.getModWin().amsg.append("\nReceived ACK " + snd_ackno
                            + " from " + packet.getFromId());
            ...
            if (snd_frameno == -1 || snd_receivedPacket) {
                if (noOfPorts != 0) {
                    ...
                    oPhysicalBuffer.add(new Packet(snd_targetSlNo, serialNo,
                            10, String.valueOf(snd_frameno)));
                    resetSendTimer();
                    modUI.getModWin().amsg.append("\nSent data packet "
                            + snd_frameno + " to " + snd_targetSlNo);
                ...
                if (timeoutTimer <= 0) {
                    if (noOfPorts != 0) {
                        oPhysicalBuffer.add(new Packet(snd_targetSlNo,
                                serialNo, 10, String.valueOf(snd_frameno)));
                        resetSendTimer();
                        modUI.getModWin().amsg
                                .append("\nTimer timed-out. Re-sent data packet "
                                        + snd_frameno + " to " + snd_targetSlNo);
                    }
                }
            }
        ...
        { // RECEIVER MODE
                ...
                if (!packet.isCorrupt() && packet.getData() != null
                    ...
                    if (tframeno == rcv_frameno)
                        ...
                        modUI.getModWin().amsg.append("\nReceived data packet "
                                + rcv_frameno + " from " + packet.getFromId());
                    }
            ...
            if (rcv_receivedPacket) {
                if (noOfPorts != 0) {
                    ...
                    Packet ack = new Packet(ack_targetSlNo, serialNo, 10,
                            String.valueOf(rcv_ackno));
                    ack.isReply = true;
                    oPhysicalBuffer.add(ack);
                    modUI.getModWin().amsg.append("\nSent ack packet "
                            + rcv_ackno + " to " + ack_targetSlNo);
