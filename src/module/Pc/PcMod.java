    ...
    private boolean stepApplication(double currtime) {
        if (iPhysicalBuffer.size() > 0) {
            ...
            if (packet.getData().startsWith("ping")) {
                if (pingData != null
                        && packet.getData().compareTo(
                                "ping" + pingData + "ping" + pingData) == 0) {
                    modUI.getModWin().amsg.append("\n" + "Reply from "
                            + packet.getFromId() + " time= "
                            + (absoluteTime - pingTime));
                    pingData = null;
                } else if (packet.getData().lastIndexOf("ping") < 3) {
                    modUI.getModWin().amsg.append("\n Ping from "
                            + packet.getFromId());
                    oPhysicalBuffer.add(new Packet(packet.getFromId(),
                            serialNo, 10, packet.getData() + packet.getData(),
                            0, true));
                }
            }
        }
        if (pingData == null && absoluteTime % (serialNo * 50) == 0) {
            ...
            oPhysicalBuffer.add(new Packet(toid, serialNo, 10, "ping"
                    + pingData, 0));
            modUI.getModWin().amsg.append("\nPinging " + toid
                    + " with 10 bytes of data");
        }
