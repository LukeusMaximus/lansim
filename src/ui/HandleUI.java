...
public class HandleUI extends ComponentUI {
    ...
    public ComponentUI owner; // Cannot be null. If it lies naked on the wire
                              // then its owner is WireUI, else has owners like
                              // ModuleUI.
    ...
    public boolean isClicked(Point pt) {
        ...
        boolean ret = (new Rectangle(X - relaxPosBy, Y - relaxPosBy, Dimension,
                Dimension)).contains(pt);
    ...
    /**
     * If the owner is instance of WireUI then returns true. This method is
     * named is_naked() because this is only when the handle is rendered.
     * 
     * @return
     */
    ...
    public void render(Graphics g) {
        if (owner instanceof WireUI) {
            ...
            g.fillRect((int) (coord.x - dimension / 2),
                    (int) (coord.y - dimension / 2), dimension, dimension);
        }
    }
