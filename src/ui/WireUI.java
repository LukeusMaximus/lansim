...
public class WireUI extends ComponentUI {
    ...
    private void updateDataFlowPacketsDis() {
        dataFlowPacketsDis = Math
                .round((length / (wire.getLatency() * managerUI.WireUI2WireSteppingRatio)));
    }
    ...
    private SegmentCoords findSegment(Point2D.Double p, SegmentCoords ignoreThis) {
        for (int i = 0; i < coords.size(); i++) {
            if (coords.get(i).liesOn(p)
                    && (ignoreThis == null || !coords.get(i).equals(ignoreThis)))
                return coords.get(i);
        }
        return null;
    }
    ...
    private Point2D.Double snap2segEnd(Point2D.Double pt, SegmentCoords seg) {
        if (seg.p1.getCoord().x - pt.x == seg.p2.getCoord().x - pt.x) {
            if (Math.abs(seg.p1.getCoord().y - pt.y) <= Math.abs(seg.p2
                    .getCoord().y - pt.y))
                return seg.p1.getCoord();
            else
                return seg.p2.getCoord();
        } else {
            if (Math.abs(seg.p1.getCoord().x - pt.x) < Math.abs(seg.p2
                    .getCoord().x - pt.x))
                return seg.p1.getCoord();
            else
                return seg.p2.getCoord();
     ...
     * 
     * @param mode
     *            The mode in which this function has been called. It can be any
     *            of the 4 modes.
     * 
     */
        ...
        if (mode == Mode.SIMULATION_MODE) {
            if (wire.hasData() || wire.hasDroppedPackets()) {
                ...
                for (int i = 0; i < packets.size(); i++) {
                    ...
                    if (dataUI.pos.x == -1) {// i.e. this dataUI has just been
                                             // created by ModuleUI and needs to
                                             // be pos.ed properly.
                        dataUI.pos = getModuleCoord(packet.getFromPort());
                        SegmentCoords segment = findSegment(dataUI.pos);
                        if (segment == null) {
                            System.out
                                    .println("Internal Error!!! No line segment for the current wire found with the location ("
                                            + dataUI.pos.toString()
                                            + ") on it.");
                            return;
                        }
                        segment.setDirection(
                                dataUI,
                                dataUI.pos.equals(segment.p1.getCoord()) ? segment.p2
                                        .getCoord() : segment.p1.getCoord());
                        ...
                        if (segment.isEndReached()) {
                            ...
                            if (segment != null) { // segment==null when data
                                                   // packet has reached at the
                                                   // other end of wire (i.e.
                                                   // the target
                                // module).
    ...
    public void render(Graphics g) {
        ...
        g.setColor(tc);
        if (managerUI.getMode() == Mode.SIMULATION_MODE
                || managerUI.getMode() == Mode.PAUSED_MODE) {
            ArrayList<Packet> packets = wire.getPackets();
            if (wire.hasDroppedPackets())
                packets.addAll(wire.getDroppedPackets());
            for (int i = 0; i < packets.size(); i++) {
                ...
                if (tempDataUi != null) {
                    ...
                    System.out
                            .println("tmpDataUI is null! Couldn't render data packet.");
            }
        }
    }
    ...
    public void render(Graphics g, Point2D.Double pointerPos) {
        if (managerUI.getMode() != Mode.WIRE_CREATION_MODE) {
            System.out
                    .println("Error: Invalid mode for this render() method of WireUI method. WIRE_CREATION_MODE needed.");
            return;
        }
        ...
        // g.drawLine((int)currentCoord.x, (int)currentCoord.y,
        // (int)pointerPos.x, (int)pointerPos.y);
        UtilGraphics.drawThickLine(g, (int) currentCoord.x,
                (int) currentCoord.y, (int) pointerPos.x, (int) pointerPos.y,
                wireWidth, normalColour);
    }

    /**
     * This is called to finalise the creation of line/wire-segment. This
     * creates a segment ending with a node.
     * 
     */
    public Handle[] commit(Point2D.Double clickPos) {
        if (managerUI.getMode() != Mode.WIRE_CREATION_MODE) {
            System.out
                    .println("Error: Invalid mode for this commit(Point2D.Double) method of WireUI method. WIRE_CREATION_MODE needed.");
            return null;
        }
    ...
    public Handle[] commit(Port p) {
        if (managerUI.getMode() != Mode.WIRE_CREATION_MODE) {
            System.out
                    .println("Error: Invalid mode for this commit(Port) method of WireUI method. WIRE_CREATION_MODE needed.");
            return null;
        }
        ...
        SegmentCoords s = new SegmentCoords(lastHandle, new Handle(
                getModuleCoord(p), true));
    ...
    /**
     * This is called when another wire is clicked during wire creation. This
     * means now both these 2 wires should be merged. The wire in creation mode
     * is passed the wire with which it is to be merged. So, actually the
     * already created wire is merged into it. The coords arraylist contents of
     * the target is appended below this present one's list.
     * 
     * @param clickPos
     * @param w
     */
    /*
     * public void commit(Point2D.Double clickPos, WireUI w){
     * if(managerUI.getMode() != Mode.WIRE_CREATION_MODE){
     * System.out.println("Error: Invalid mode for this commit(Point2D.Double,
     * WireUI) method of WireUI method. WIRE_CREATION_MODE needed."); return; }
     * SegmentCoords s = new SegmentCoords(new Handle(currentCoord), new
     * Handle(clickPos)); s.setEndPorts(currentPort, null); coords.add(s);
     * currentCoord = null; for(int i=0;i<w.coords.size();i++)
     * coords.add(w.coords.get(i)); wire.merge(w.wire); //length += s.segLen();
     * //dataFlowPacketsDis=
     * length/(lenOfWireGuareentedToBeAbleShowFullAimation/dataFlowPacketsDis);
     * //Now ManagerUI should reset the mode to EDIT_MODE. }
     */

    /**
     * Locates the entry in coords with h. It returns its index in coords list.
     * If no occurances are found then -1 is returned.
     * 
     * @param h
    ...
    /**
     * Returns the Handle other than the one passed to it at the row given by
     * index.
     * 
     * @param index
     * @param h
    ...
    private Handle getTheOtherHandle(int index, Handle h) {
        return coords.get(index).p1.equals(h) ? coords.get(index).p2 : coords
                .get(index).p1;
    }

    public boolean isClicked(Point2D.Double pt) {
        for (int i = 0; i < coords.size(); i++) {
            if (coords.get(i).liesOn(pt)) // i.e. This wire has been clicked if
                                          // the clicked pt lies on any of the
                                          // line segments making it.
                return true;
        }
        return false;
    }
    ...
    public boolean isClicked(Point pt) {
        for (int i = 0; i < coords.size(); i++) {
            ...
            if ((coords.get(i)).liesOn(ptD)) // i.e. This wire has been clicked
                                             // if the clicked pt lies on any of
                                             // the line segments making it.
                return true;
        }
        return false;
    }
    ...
    /**
     * Removes only 'naked' handles, i.e. Handles that are not associated with
     * any ports.
     * 
     * @param handle
     */
    public void removeNakedHandle(Handle h) {
        if (h.isPort()) {
            System.out
                    .println("Error! Handle to remove is not naked. Ignoring request.");
            return;
        }
        int i = locate(h);
        if (i == -1) {
        ...
        /*
         * Since a naked handle will always be in some segments' p1 and p2 both,
         * and, since, locate() scans from the top hence it will return the
         * segment where this handle is on p2. So, we need not use
         * getAdjacentHandles().
         */
    ...
    // Class---------------------------------------------------------------------------------------------
    /**
     * Data structure to store the coords of the two end points of a line
     * segment.
     * 
     * @author Niruam
     * 
    ...
    private class SegmentCoords {
        ...
        /**
         * Updates the values of some private data that are pre-calculated
         * during segment's creation for optimisation. The users must call this
         * to update.
         */
        public void updatePrivate() {
            ...
            if (a.getCoord().x == b.getCoord().x) {
                ...
                slope = ((b.getCoord().y - a.getCoord().y) / (b.getCoord().x - a
                        .getCoord().x));
                sqrtOfSlopeSquarePlus1 = Math.sqrt(slope * slope + 1); // Used
                                                                       // by
                                                                       // getNxtXYfor()
                infinity = false;
            }
        ...
        /**
         * Same as liesOn(Point2D.Double,int) but uses wire width defined at the
         * top of WireUI class.
         * 
         * @param pt
         * @return
        ...
        boolean liesOn(Point2D.Double pt) {
            return liesOn(pt, wireWidth); // i.e. it defaults to the wire width
                                          // defined at the top of WireUI class.
        }
        ...
        boolean liesOn(Point2D.Double pt, double wireWidth) {
            ...
            if (y < yy - wireWidth || y > yy + wireWidth) {
                // System.out.println("for y  " + (yy - wireWidth) + "  " + (yy
                // + wireWidth) + " " + y + " where x,x1,x2=" + x + " " + x1 +
                // " " + x2);
                return false;
            } // This is an approximation where actually we are subracting from
              // c of y=mx+c.
              // This will only hold for small values of wireWidth. For large
              // values c can't
            else
                // be taken along the width of wire. Actually the width of wire
                // is along a line
                return true; // perpendicular to the actual line, extending half
                             // the width of wire on both sides.
        }

        /**
         * Returns the value of y lying on this line segemnt for a given x. When
         * slope is infinity, i.e the line is parallel to y-axis then 0 is
         * returned and sets infinity flag to true. Use slopeInfinity() to check
         * it.
         * 
         * @param x
         * @return
        ...
        /**
         * Sets flow direction of data in this segment given the 'from' and 'to'
         * coordinates of two different points on this segment.
         * 
         * @param from
         * @param to
         */
        void setDirection(DataUI dataUi, Point2D.Double to) {
            ...
            if (direc < 0)
                ...
                System.out
                        .println("Internal Error!!! Value of both y's OR x's are same. So direction is zero. Infinity = "
                                + infinity);
            dataUi.setDirection(direction);
        }

        /**
         * Gives the value of next x,y for the given x,y while taking into
         * consideration the dataFlowPacketsDis and slope of this segment.
         * 
         * @param curr
         * @return
         */
        Point2D.Double getNxtXYfor(DataUI dataUi) {
            ...
            if (direction == 0) {
                System.out
                        .println("Internal Error!!! Segment method getNxtXYfor() called without setting the direction of data packet flow!");
                return null;
            } else
         ...
         * @param curr
         * @param direc
         *            This decides the direction in which to move (i.e.
         *            increment or decrement it). -ve or positive value.
         * @return
         */
        Point2D.Double getNxtXYfor(Point2D.Double curr, int direc) {
            ...
            if (!liesOn(p)) { // The generated coords are out of this line
                              // segement so snap it to one of the ends.
                endReached = true;
        ...
        boolean equals(SegmentCoords s) {
            if ((p1.equals(s.p1) && p2.equals(s.p2))
        ...
        void render(Graphics g) {
            // g.drawLine((int)p1.getCoord().x, (int)p1.getCoord().y,
            // (int)p2.getCoord().x, (int)p2.getCoord().y);
            Color colour = normalColour;
            if (wire.isDroppingPackets())
                colour = dropPacketColour;
            UtilGraphics.drawThickLine(g, (int) p1.getCoord().x,
                    (int) p1.getCoord().y, (int) p2.getCoord().x,
                    (int) p2.getCoord().y, wireWidth, colour);
        }
    }
}