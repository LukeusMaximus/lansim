...
/**
 * It handles the rendering of client space. It is also responsible for handling
 * the events generated by client space.
 * 
 * @author Nirupam
 * @author Rohit
 * 
...
public class ManagerUI extends JPanel implements Runnable {
    ...
    private Thread t;
    private Rectangle boundingbox = null; // Stores the reactangle within which
                                          // all ComponentUI objects lie. Used
                                          // for
                                          // calculating min. size of
    // client area.
    final int WireUI2WireSteppingRatio = 3; // Used to make data flow animation
                                            // smoother without increasing
                                            // latency of wires.
    ...
    /**
     * Signals ManagerUI to start/restart its thread. Calling startthread() will
     * automatically set renderAllways to true.
     */
    public void startthread() {
        ...
        if (t != null) {
            ...
            try {
                ...
                System.out
                        .println("Failed to join to ManagerUI Thread before starting another one.");
                e.printStackTrace();
            }
        }
        terminateManagerUI = false;
    ...
    /**
     * Enabling renderAllways will make ManagerUI continuously display frames at
     * simulation FPS, irrespective of any modes. This is required if there are
     * animated gifs in the client space, otherwise, turn it off this is a huge
     * processing hog. Calling startthread() will automatically set
     * renderAllways to true.
     * 
     * @param b
     */
    public void setRenderAllways(boolean b) {
        if (b) {
            if (renderAllways)
                return;
            startthread();// It will automatically set renderAllways to true and
                          // terminate current thread if running and restart it.
        } else {
            if (!renderAllways)
                return;
            renderAllways = false;
    ...
    /**
     * Updates value of boundingbox. Call this whenever any ComponentUI objects
     * move or are added or removed.
     * 
     */
    ...
    private synchronized void addHandle(Handle handle, ComponentUI owner,
            WireUI AssociatedWireUI) {
    ...
    private synchronized void rePositionHandles(ComponentUI componentui,
            Point2D.Double loc) {
        if (componentui != null) {
            for (int i = 0; i < handlesUI.size(); i++) {
    ...
    private synchronized void mergeHandle(HandleUI h) {
        ...
        for (int i = 0; i < handlesUI.size(); i++) {
            ...
            if (hi.isClicked(h.handle.getCoord())) {// Don't get misguided by
                                                    // isClciked name. It could
                                                    // had been named
                                                    // isPointLiesWithin.
    ...
    class ManagerUIMouseListener extends java.awt.event.MouseAdapter {
        ...
        private void getNewModToBuffer() {
            if (selectionName != null && buffer.EditMode.module == null
                    || buffer.EditMode.moduleUI == null) {
                Class newClass;
                if (loadedModuleNames.contains(selectionName)) {
                    newClass = loadedModules.get(loadedModuleNames
                            .indexOf(selectionName));
                } else {
                    try {
                        // newClass=loader.load(selectionName);
                        // Loader (our custom classloader in Loader.java) has
                        // been phased out, since this is simpler and
                        // that was making JVM throw
                        // "LinkageError [classname] violates loader constraints"
                        // exception when anyone tried to access member
                        // functions or variables from objects
                        // of classes loaded by Loader classloader.

                        // String className = selectionName.replaceAll("/",
                        // ".");// Converting Pathname to classname
                        // className = className.substring(0,
                        // className.lastIndexOf('.'));
                        String className = MiscFunctions
                                .convertPath2className(selectionName);
                        ...
                        if (selectionName != null) {
                            System.out.println("Error while loading module: "
                                    + selectionName);
                            Le.printStackTrace();
                        }
                        return;
                ...
                try {
                    ...
                    System.out.println("Error while instantiating module: "
                            + selectionName);
                    Ie.printStackTrace();
                }
                if (buffer.EditMode.module.getModuleUI() == null)
                    buffer.EditMode.module.setModuleUI(new ModuleUI(
                            buffer.EditMode.module));
        ...
        public void mousePressed(java.awt.event.MouseEvent e) {
            if (e.getButton() == MouseEvent.BUTTON1) {
                switch (mode) {
                case EDIT_MODE:
                    // This has been done so that the components donot get
                    // dragged when dragging has started from some
                    // place other than the intended component. This prevents
                    // unintended dragging, when the user by mistake
                    // drags out the mouse.
                    for (int i = 0; i < handlesUI.size(); i++) {
        ...
        public void mouseClicked(java.awt.event.MouseEvent e) {
            ...
            if (e.getButton() == MouseEvent.BUTTON3) {
                ...
                case EDIT_MODE:
                    selectionName = null;
                    if (buffer.EditMode.module != null
                            || buffer.EditMode.moduleUI != null) { // CANCEL
                                                                   // SELECTION
                ...
                if (isDoubleClick(e)) {// ----HANDLE DOUBLE CLICK
                    ...
                    case WIRE_CREATION_MODE:
                        ...
                        for (int i = 0; i < modulesUI.size(); i++) {
                            if (modulesUI.get(i).isClicked(e.getPoint())) {
                                modulesUI.get(i).setStatus(ModuleUI.NONE);
                                if (buffer.WireCreationMode.port.getOwner() != modules
                                        .get(i)) {
                                    buffer.WireCreationMode.port
                                            .getOwner()
                                            .addPort(
                                                    buffer.WireCreationMode.port,
                                                    0);
                                    Port p = new Port("Ethernet",
                                            modules.get(i));

                                    modules.get(i).addPort(p, 0);
                                    buffer.WireCreationMode.wire.addPort(p);
                                    Handle[] h = buffer.WireCreationMode.wireUI
                                            .commit(p);
                                    // modulesUI.get(i).addHandle(h[0]);
                                    if (h[0].isPort())
                                        addHandle(
                                                h[0],
                                                getModuleUI(buffer.WireCreationMode.port
                                                        .getOwner()),
                                                buffer.WireCreationMode.wireUI);
                                    /*
                                     * else
                                     * addHandle(h[0],buffer.WireCreationMode
                                     * .wireUI);
                                     */
                                    addHandle(h[1], modulesUI.get(i),
                                            buffer.WireCreationMode.wireUI);
                        ...
                        if (!flag) {
                            // Handle h=
                            // buffer.WireCreationMode.wireUI.commit(new
                            // Point2D.Double(e.getX(),e.getY()));
                            Handle h[] = buffer.WireCreationMode.wireUI
                                    .commit(new Point2D.Double(e.getX(), e
                                            .getY()));
                            // if(h!=null)
                            // getModuleUI(buffer.WireCreationMode.wire.getPort(0).getOwner()).addHandle(h);
                            if (h[0].isPort())
                                addHandle(
                                        h[0],
                                        getModuleUI(buffer.WireCreationMode.wire
                                                .getPort(0).getOwner()),
                                        buffer.WireCreationMode.wireUI);
                            /*
                             * else
                             * addHandle(h[0],buffer.WireCreationMode.wireUI);
                             */
                    ...
                    case EDIT_MODE:
                        ...
                        if (buffer.EditMode.module != null
                            ...
                            System.out
                                    .println(buffer.EditMode.module.getName());
                            buffer.EditMode.module = null;
                            if (buffer.EditMode.moduleUI.getModWin() == null) {
                                buffer.EditMode.moduleUI
                                        .setModWin(new moduleWindow(
                                                buffer.EditMode.moduleUI));
                            }
                            ...
                            for (int i = 0; i < modulesUI.size(); i++) {
                                if (modulesUI.get(i).isClicked(e.getPoint())) {
                                    modulesUI.get(i).setStatus(
                                            ModuleUI.SELECTED);
                                    mode = Mode.WIRE_CREATION_MODE;
                                    buffer.WireCreationMode.port = new Port(
                                            "Eternet", modules.get(i));
                                    buffer.WireCreationMode.wire = new Wire(
                                            buffer.WireCreationMode.port);
                                    buffer.WireCreationMode.wireUI = new WireUI(
                                            buffer.WireCreationMode.wire,
                                            getReference());
    ...
    class ManagerUIMouseMotionListener extends
        ...
        public void mouseMoved(java.awt.event.MouseEvent e) {
            ...
            case WIRE_CREATION_MODE:
                ...
                buffer.WireCreationMode.mousePointerLocation = new Point2D.Double(
                        e.getX(), e.getY());
                if (!renderAllways)
                    render();
                break;
        ...
        public void mouseDragged(java.awt.event.MouseEvent e) {
            ...
            if (!locInBound(e.getPoint())) {
                if (mode == Mode.EDIT_MODE && e.getPoint().x > 0
                        && e.getPoint().y > 0) {
                    boolean flag = false;
                    for (int i = 0; i < modulesUI.size(); i++)
                        if (modulesUI.get(i).isClicked(e.getPoint())
                                || modulesUI.get(i).getStatus() == ModuleUI.MOVING) {
                            flag = true;
                        }
                    if (!flag)
                        for (int i = 0; i < handlesUI.size(); i++)
                            if (handlesUI.get(i).isClicked(e.getPoint())
                                    || handlesUI.get(i).getStatus() == HandleUI.MOVING) {
                                flag = true;
                            }
                    if (!flag)
                        return;
            ...
            switch (mode) {
            case WIRE_CREATION_MODE:
                buffer.WireCreationMode.mousePointerLocation.setLocation(e
                        .getPoint());
                if (!renderAllways)
                    render();
                break;
            ...
            case EDIT_MODE:
                for (int i = 0; i < handlesUI.size(); i++) {
                    if (handlesUI.get(i).is_naked()) {// Moves only 'naked'
                                                      // handles.
                        if (noOneMoving
                                && handlesUI.get(i).isClicked(e.getPoint())) {
                            if ((handlesUI.get(i).getStatus() != HandleUI.MOVING)
                                    && handlesUI.get(i).getStatus() == ModuleUI.SELECTED)
                ...
                for (int i = 0; i < modulesUI.size(); i++) {
                    if (noOneMoving && modulesUI.get(i).isClicked(e.getPoint())) {
                        if ((modulesUI.get(i).getStatus() != ModuleUI.MOVING && modulesUI
                                .get(i).getStatus() == ModuleUI.SELECTED))
                            modulesUI.get(i).setStatus(ModuleUI.MOVING);
                        noOneMoving = false;
                    }
                    if ((modulesUI.get(i).getStatus() == ModuleUI.MOVING)) {
                        modulesUI.get(i).setCoord(e.getPoint());
                        rePositionHandles(modulesUI.get(i), modulesUI.get(i)
                                .getDoubleCoord());
    ...
    private boolean locInBound(Point p) {
        int x = p.x, y = p.y;
        if (y >= 0 && y <= (this.getSize().height - 30) && x >= 0
                && x <= (this.getSize().width - 30)) {
            return true;
        } else {
            return false;
        }
    ...
    public void run() {
        ...
        long period = (long) (((double) 1000) / FPS); // Amount of time a frame
                                                      // maybe displayed, in ms.
    ...
    public boolean stepSimulation(int stepping) {
        /*
         * if(mode!=Mode.SIMULATION_MODE) return false; if(clock.tick()){
         * manager.stepSimulation(); //The following objects take the decision
         * of stepping themselves based on mode of ModuleUI. //NOTE: ModuleUI
         * MUST be called before wireUI is called because it creates the dataUI
         * object that is used by wireUI. for(int i=0;i<modulesUI.size();i++)
         * modulesUI.get(i).stepSimulation(this.mode); for(int
         * i=0;i<wiresUI.size();i++) wiresUI.get(i).stepSimulation(this.mode);
        ...
        // The following objects take the decision of stepping themselves based
        // on mode of ModuleUI.
        // NOTE: ModuleUI MUST be called before wireUI is called because it
        // creates the dataUI object that is used by wireUI.
        if (mode != Mode.SIMULATION_MODE || clocktick) {
            if (stepping == WireUI2WireSteppingRatio
                    || mode != Mode.SIMULATION_MODE) {
                for (int i = 0; i < modulesUI.size(); i++)
    ...
    /**
     * It calls renderToBuf first to render to buffer then calls activePaint to
     * render it to screen.
     * 
     */
    public void render() {
    ...
    /**
     * It renders without calling renderBuf(). Useful to save time when
     * paintComponent is called.
     * 
     */
    /*
     * public void renderWithoutUpdate(){ if(simBuffer==null) renderToBuf();
     * activePaint(); }
     */

    /**
     * Renders to buffer. This is thread safe. Done to keep safe simBuffer
     * during multiple calls from ManagerUI thread and AWT event dispacher
     * thread.
     */
        ...
        if (mode == Mode.EDIT_MODE) {
            ...
            if (buffer.WireCreationMode.wireUI != null) {
                buffer.WireCreationMode.wireUI.render(g,
                        buffer.WireCreationMode.mousePointerLocation);
                try {
                    buffer.WireCreationMode.wireUI.render(g);
    ...
    /**
     * Active rendering of buffer to screen. This is thread safe. Done to keep
     * safe simBuffer during multiple calls from ManagerUI thread and AWT event
     * dispacher thread.
     */
    /*
     * public void activePaint(){ Graphics g; try { //g = this.getGraphics( );
     * // get the panel's graphic context if ((g != null) && (simBuffer !=
     * null)) g.drawImage(simBuffer, 0, 0, null); Toolkit.getDefaultToolkit(
     * ).sync( ); // sync the display on some systems if (g != null) g.dispose(
        ...
        if (!renderAllways) {
            if (t != null) {
                try {
                    ...
                    System.out
                            .println("Err trying to wait for render thread to die.");
                    e.printStackTrace();
                }
            }
            render();
