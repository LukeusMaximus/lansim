...
/**
 * Wire.java Represents a link in the network can be connected to any number of
 * Ports. Created on December 7, 2006,
 * 
 * @author Nirupam
 * @author Rohit
 */
public class Wire {
    ...
    // private double dpu = 1; //Data packet transfer rate per unit time. Take
    // this as a convinient factor times actual bps of a connection.
    ...
    /**
     * If set to true then all packets from that point of time will be dropped.
     * These dropped packets will then be put into dropped_packet ArrayList.
     * 
     * @param state
     */
        ...
        for (int p = 0; p < packets.size(); p++) {
            ...
            if ((currtime - packets_in_time.get(p)) >= LATENCY) {
                for (int i = 0; i < noOfPorts; i++) {
                    if (ports[i].isActive()
                            && (ports[i] != tempPacket.getFromPort())) {
    ...
    private void getPacketFromPorts(double currtime) {
        for (int i = 0; i < dropped_packets.size();)
            ...
            } else if (!((DroppedPacketDataUI) dropped_packets.get(i).dataUI)
                    .isAlive())
                    ...
                    if (dropPackets) {
                        ...
                        if (this.pktAvailable)
                            for (int j = 0; j < packets.size(); j++) {
                                if (packets.get(j).getFromPort() != tempPacket
                                        .getFromPort()) {
